...
public final class LightningAssemblyRecipeService {
    ...
    private static final Comparator<LightningAssemblyRecipe> RECIPE_ORDER = Comparator
            .<LightningAssemblyRecipe>comparingInt(holder -> holder.priority())
            .reversed()
            .thenComparing(Comparator.comparingInt(
                    (LightningAssemblyRecipe holder) -> holder.inputs().size()).reversed())
            .thenComparing(Comparator.comparingInt(
                    (LightningAssemblyRecipe holder) -> holder.totalInputCount()).reversed())
            .thenComparing(holder -> holder.getId().toString());

    private static List<LightningAssemblyRecipe> cachedRawRecipeList;
    ...
    private static int computeRecipeOrderFingerprint(List<LightningAssemblyRecipe> recipes) {
        int hash = 1;
        for (var holder : recipes) {
            var recipe = holder;
            hash = 31 * hash + holder.getId().hashCode();
    ...
    public static Optional<LightningAssemblyRecipe> findRecipeById(Level level, ResourceLocation recipeId) {
        ...
        for (LightningAssemblyRecipe recipe
