...
public interface FrequencyBindingAccess {
     ...
     * (for example {@code GRID_BOOT}); this triggers a reconnection attempt.
     */
    void onMainNodeStateChanged(IGridNodeListener.State reason);

    /** Append the bound frequency id to the BE's saved data (key {@code "FrequencyId"}). */
    void save(ValueOutput output);

    /** Restore the bound frequency id from the BE's saved data. */
    void load(ValueInput input);

    /** Append the bound frequency id to a raw NBT tag (key {@code "FrequencyId"}). */
    void save(CompoundTag tag);

    /** Restore the bound frequency id from a raw NBT tag. */
