...
public final class WirelessConnectionValidator {
    ...
    public static Status validate(ServerLevel hostLevel, BlockPos hostPos, WirelessConnectionRef target) {
    ...
    public static Status validate(
        ...
        var chunk = targetLevel.getChunkSource().getChunkNow(targetPos.getX() >> 4, targetPos.getZ() >> 4);
        if (chunk == null) {
            return Status.UNLOADED;
        }
        if (chunk.getBlockEntity(targetPos) == null) {
            return Status.REMOVE;
        }
        return Status.VALID;
    }
