...
* **由控制器決定網絡總頻道**：整個過載網絡的頻道總量取決於網絡中**過載 ME 控制器**的數量——每個過載控制器為網絡提供 128 頻道（預設值，可設定）
* **智慧型頻道分配**：過載網絡使用**最大流演算法**替代原版 AE2 的 BFS 頻道分配，在複雜拓撲下能更高效地利用總頻道容量
* **密集線纜外觀**：視覺上與原版密集線纜一致
...
2. **網絡總頻道 = 過載控制器數 × 128**：整個網絡可用的頻道總量由過載 ME 控制器的數量決定
3. **最大流分配**：可用頻道在網絡中按最大流演算法智慧分配，而非沿路徑逐段扣減

對玩家而言意味著：

* 不再需要擔心某條主幹線纜的頻道被用完
* 只要網絡總頻道足夠，所有設備都能獲得頻道
* 增加過載 ME 控制器即可擴展網絡的頻道總量
...
白色、橙色、洋紅色、淺藍色、黃色、淺綠色、粉紅色、灰色、淺灰色、青色、紫色、藍色、棕色、綠色、紅色、黑色。

不同顏色的過載線纜互不連接（與原版 AE2 線纜染色機制一致）。

## 使用方式

過載線纜的使用方式與原版密集線纜一致。直接放置在方塊側面即可接入網絡。

## 適用場景

* 大型基地主幹線路，一根過載線纜可承擔多根密集線纜的頻道負載
* 以過載 ME 控制器為核心的高容量網絡
* 簡化網絡拓撲，不再需要針對頻道瓶頸進行布線設計
