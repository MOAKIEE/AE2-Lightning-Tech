...
public final class OverloadProcessingRecipe implements Recipe<OverloadProcessingRecipeInput> {
    ...
    public static final class Serializer implements RecipeSerializer<OverloadProcessingRecipe> {
        ...
        private static void writeToBuf(FriendlyByteBuf buf, OverloadProcessingRecipe recipe) {
            ...
            FluidStack.OPTIONAL_STREAM_CODEC.encode(buf, recipe.rawFluidInput());
            buf.writeVarInt(recipe.itemResults.size());
            for (ItemStack stack : recipe.rawItemResults()) {
                buf.writeItem(stack);
            }
            FluidStack.OPTIONAL_STREAM_CODEC.encode(buf, recipe.rawFluidResult());
            buf.writeVarLong(recipe.totalEnergy);
            buf.writeVarInt(recipe.lightningCost);
            recipe.lightningTier.writeToBuf(buf);
        }

        private static OverloadProcessingRecipe readFromBuf(FriendlyByteBuf buf) {
            int priority = buf.readVarInt();
            int inputSize = buf.readVarInt();
            List<OverloadProcessingIngredient> itemInputs = new ArrayList<>(inputSize);
            for (int i = 0; i < inputSize; i++) {
                itemInputs.add(OverloadProcessingIngredient.readFromBuf(buf));
            }
            FluidStack fluidInput = FluidStack.OPTIONAL_STREAM_CODEC.decode(buf);
            int outputSize = buf.readVarInt();
            List<ItemStack> itemResults = new ArrayList<>(outputSize);
            for (int i = 0; i < outputSize; i++) {
                itemResults.add(buf.readItem());
            }
            FluidStack fluidResult = FluidStack.OPTIONAL_STREAM_CODEC.decode(buf);
            long totalEnergy = buf.readVarLong();
            int lightningCost = buf.readVarInt();
            LightningKey.Tier lightningTier = LightningKey.Tier.readFromBuf(buf);
